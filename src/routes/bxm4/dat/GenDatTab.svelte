<script lang="ts">
	...
	// 거래코드 정규식 패턴
	const TRX_CD_REGEX = /^SMYP\d{4}[A-Z]\d{3}$/;
	...
	const realRegister = async (): Promise<void> => {
		const response = await fetch('/bxm4/dat/register/stream', {
			method: 'POST',
			headers: {
